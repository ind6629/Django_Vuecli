...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        const res = await await request.get(
          "http://localhost:8000/Backend_Store/api/realtime_searchData"
        );
    ...
    updateChart() {
      ...
      const counts = sortedRecords.map((r) => r.times);
...
<style src="../../assets/css/Realtime_SearchData.css" scoped></style>
