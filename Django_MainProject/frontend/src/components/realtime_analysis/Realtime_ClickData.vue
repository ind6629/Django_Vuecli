...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        const res = await await request.get(
          "http://localhost:8000/Backend_Store/api/realtime_clickData"
        );
    ...
    updateChart() {
      ...
      const datax = this.products.data.map((p) => p.category);
      const datay = this.products.data.map((p) => p.times);
...
<style src="../../assets/css/Realtime_ClickData.css" scoped></style>